...
import { ArrowLeft, CheckCircle2, MapPin, MessageCircle, Package, ShoppingBag, User as UserIcon } from 'lucide-react';
...
export default function CheckoutPage() {
    ...
    const { items, getTotal, getTotalItems, isInitialized, clearCart } = useCart();
    ...
    }, [isAuthenticated, authLoading, items.length, router, isInitialized, saleCreated]);
        ...
        if (isAuthenticated && user?.id && !addressLoaded) {
            ...
            getUserWithAddressesAction(user.id).then((userData) => {
                if (userData && userData.enderecos && userData.enderecos.length > 0) {
                    ...
                    const enderecoPrincipal = userData.enderecos.find((e: Address) => e.principal) || userData.enderecos[0];
    ...
    return (
        <div className="bg-gray-100 min-h-screen">
            ...
            <div className="container mx-auto px-4 py-6 max-w-7xl">
                <h1 className="text-3xl font-bold text-gray-900 mb-6">Finalizar Pedido</h1>
                                ...
                                {userAddress ? (
                                    <div className="space-y-2">
                                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                            ...
                                            {userAddress.complemento && (
                                                <p className="text-sm text-gray-600">{userAddress.complemento}</p>
                                            )}
                                            <p className="text-sm text-gray-600">{userAddress.bairro}</p>
                                            <p className="text-sm text-gray-600">
                                                {userAddress.cidade} - {userAddress.estado}
                                            </p>
                                            <p className="text-sm text-gray-600">CEP: {userAddress.cep}</p>
                                        </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-sm text-gray-500">
                                            O endereço de entrega será solicitado via WhatsApp com nossa equipe.
                                        </p>
                                    </div>
                                )}
                            </CardBody>
                            ...
                            <CardBody className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {items.map((item) => {
                                        ...
                                        return (
                                            <div key={`${item.product.id}-${item.tipoVenda}`} className="flex gap-3">
                            ...
                            <Card className="bg-green-50 border-2 border-green-600">
                                <CardBody className="p-6 text-center space-y-3">
                                    ...
                                    <p className="text-sm text-green-700">
                                        Número do pedido: <span className="font-mono font-bold">#{saleNumber}</span>
                                    </p>
                                ...
                                <CardBody className="p-4">
                                    <div className="flex items-start gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-green-700">
                                                Finalize seu pedido diretamente com nossa equipe pelo WhatsApp.
                                                Confirmaremos detalhes de entrega e pagamento.
                                            </p>
                                        </div>
                                    </div>
                                    <Button
