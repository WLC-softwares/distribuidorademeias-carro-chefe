...
export function AddressSelector({
    ...
    useEffect(() => {
        ...
        if (!selectedId && addresses.length > 0) {
            const primaryAddress = addresses.find((addr) => addr.primary) || addresses[0];
                            ...
                            <div className="space-y-3">
                                {displayedAddresses.map((address) => (
                                    <Radio
                                        ...
                                        value={address.id}
                                    >
                                        <div className={`w-full flex flex-col justify-between ${address.primary ? 'h-[130px]' : 'h-[110px]'}`}>
                                            <div>
                                                ...
                                                <div className="text-sm text-gray-600 space-y-0.5 mt-2">
                                                    {address.complement && (
                                                        <p className="leading-tight">{address.complement}</p>
                                                    )}
                                                    <p className="leading-tight">{address.neighborhood}</p>
